...
public class FlowController {
    ...
    public FlowGraph flowGraph(
        ...
        String flowUid = revision.isEmpty() ? Flow.uidWithoutRevision(tenantService.resolveTenant(), namespace, id)
                : Flow.uid(tenantService.resolveTenant(), namespace, id, revision);
        if (flow == null) {
    ...
    @Schema(oneOf = { FlowWithSource.class, Flow.class })
    // FIXME we return Object instead of Flow as Micronaut, since 4, has an issue
    // with subtypes serialization, see
    // https://github.com/micronaut-projects/micronaut-core/issues/10294.
    public Object index(
            @Parameter(description = "The flow namespace") @PathVariable String namespace,
            @Parameter(description = "The flow id") @PathVariable String id,
        ...
        return source ? flowRepository
                .findByIdWithSource(tenantService.resolveTenant(), namespace, id, Optional.ofNullable(revision),
                        allowDeleted)
                .orElse(null)
                : flowRepository
    ...
    // FIXME we return Object instead of Task as Micronaut, since 4, has an issue
    // with subtypes serialization, see
    // https://github.com/micronaut-projects/micronaut-core/issues/10294.
    ...
    public PagedResults<SearchResult<Flow>> source(
            ...
            @Parameter(description = "A namespace filter prefix") @Nullable @QueryValue String namespace)
            throws HttpStatusException {
        return PagedResults.of(flowRepository.findSourceCode(PageableUtils.from(page, size, sort), query,
                tenantService.resolveTenant(), namespace));
    }
    ...
    public HttpResponse<FlowWithSource> create(
            @Parameter(description = "The flow") @Body String flow) throws ConstraintViolationException {
        Flow flowParsed = yamlFlowParser.parse(flow, Flow.class);
    ...
    public HttpResponse<Flow> create(
            @Parameter(description = "The flow") @Body Flow flow) throws ConstraintViolationException {
    ...
    @Operation(tags = {
            "Flows" }, summary = "Update a complete namespace from yaml source", description = "All flow will be created / updated for this namespace.\n"
                    +
    ...
    @Post(uri = "{namespace}")
    @Operation(tags = {
            "Flows" }, summary = "Update a complete namespace from json object", description = "All flow will be created / updated for this namespace.\n"
                    +
                    "Flow that already created but not in `flows` will be deleted if the query delete is `true`")
    ...
    protected List<FlowWithSource> updateCompleteNamespace(String namespace, List<FlowWithSource> flows,
            Boolean delete) {
        ...
        List<FlowWithSource> updatedOrCreated = flows.stream()
                .map(flowWithSource -> {
                    ...
                    Optional<Flow> existingFlow = flowRepository.findById(tenantService.resolveTenant(), namespace,
                            flow.getId());
                    if (existingFlow.isPresent()) {
                        return flowRepository.update(flow, existingFlow.get(), flowWithSource.getSource(),
    ...
    public HttpResponse<Flow> updateTask(
            ...
            @Parameter(description = "The flow id") @PathVariable String id,
            @Parameter(description = "The task id") @PathVariable String taskId,
            @Parameter(description = "The task") @Valid @Body Task task) throws ConstraintViolationException {
        Optional<Flow> existingFlow = flowRepository.findById(tenantService.resolveTenant(), namespace, id);
        ...
        if (!taskId.equals(task.getId())) {
            throw new IllegalArgumentException(
                    "Invalid taskId, previous '" + taskId + "' & current '" + task.getId() + "'");
        }

        Flow flow = existingFlow.get();
        try {
            Flow newValue = flow.updateTask(taskId, task);
            return HttpResponse.ok(flowRepository
                    .update(newValue, flow, flow.generateSource(), pluginDefaultService.injectDefaults(newValue))
                    .toFlow());
        } catch (InternalException e) {
            return HttpResponse.status(HttpStatus.NOT_FOUND);
        }
    ...
    public List<String> listDistinctNamespace(
        ...
        if (authentication.getRoles().contains("admin")) {
            ...
            List<String> namespaces = flowRepository.findDistinctNamespace(tenantService.resolveTenant(), query);

            namespaces.removeIf(ns -> {
                boolean eligible = false;
                for (String role : authentication.getRoles()) {
                    eligible = eligible || ns.startsWith(role);
                }
                return eligible;
            });
    ...
    public FlowTopologyGraph dependencies(
        ...
        List<FlowTopology> flowTopologies = flowTopologyRepository.findByFlow(tenantService.resolveTenant(), namespace,
                id, destinationOnly);
    ...
    public List<ValidateConstraintViolation> validateFlows(
        ...
        return Stream
                .of(flows.split("\\n+---\\n*?"))
                .map(flow -> {
                    ValidateConstraintViolation.ValidateConstraintViolationBuilder<?, ?> validateConstraintViolationBuilder = ValidateConstraintViolation
                            .builder();
                    ...
                    try {
                        ...
                        if (sentRevision != null) {
                            Integer lastRevision = Optional
                                    .ofNullable(flowRepository.lastRevision(tenantService.resolveTenant(),
                                            flowParse.getNamespace(), flowParse.getId()))
                                    .orElse(0);
                        ...
                        warnings.addAll(flowService.relocations(flow).stream()
                                .map(relocation -> relocation.from() + " is replaced by " + relocation.to()).toList());
                        ...
                        // In case of any error, we add a validation violation so the error is displayed
                        // in the UI.
                        // We may change that by throwing an internal error and handle it in the UI, but
                        // this should not occur except for rare cases
                        // in dev like incompatible plugin versions.
                        log.error("Unable to validate the flow", re);
                        validateConstraintViolationBuilder
                                .constraints("Unable to validate the flow: " + re.getMessage());
                    }
    ...
    @Operation(tags = { "Flows" }, summary = "Export flows as a ZIP archive of yaml sources.")
    public HttpResponse<byte[]> exportByQuery(
            @Parameter(description = "A string filter") @Nullable @QueryValue(value = "q") String query,
            @Parameter(description = "A namespace filter prefix") @Nullable @QueryValue String namespace,
            @Parameter(description = "A labels filter") @Nullable @QueryValue List<String> labels) throws IOException {
        var flows = flowRepository.findWithSource(query, tenantService.resolveTenant(), namespace,
                RequestUtils.toMap(labels));
    ...
    public HttpResponse<byte[]> exportByIds(
        ...
        var flows = ids.stream()
                .map(id -> flowRepository
                        .findByIdWithSource(tenantService.resolveTenant(), id.getNamespace(), id.getId()).orElseThrow())
                .collect(Collectors.toList());
        var bytes = zipFlows(flows);
        return HttpResponse.ok(bytes).header("Content-Disposition", "attachment; filename=\"flows.zip\"");
    }
    ...
    @Operation(tags = { "Flows" }, summary = "Delete flows returned by the query parameters.")
    ...
    @Operation(tags = { "Flows" }, summary = "Delete flows by their IDs.")
    ...
    @Operation(tags = { "Flows" }, summary = "Disable flows returned by the query parameters.")
    public HttpResponse<BulkResponse> disableByQuery(
        ...
        return HttpResponse.ok(
                BulkResponse.builder().count(setFlowsDisableByQuery(query, namespace, labels, true).size()).build());
    }

    @ExecuteOn(TaskExecutors.IO)
    @Post(uri = "/disable/by-ids")
    @Operation(tags = { "Flows" }, summary = "Disable flows by their IDs.")
    ...
    @Operation(tags = { "Flows" }, summary = "Enable flows returned by the query parameters.")
    public HttpResponse<BulkResponse> enableByQuery(
        ...
        return HttpResponse.ok(
                BulkResponse.builder().count(setFlowsDisableByQuery(query, namespace, labels, false).size()).build());
    }

    @ExecuteOn(TaskExecutors.IO)
    @Post(uri = "/enable/by-ids")
    @Operation(tags = { "Flows" }, summary = "Enable flows by their IDs.")
    ...
    @Operation(tags = {
            "Flows" }, summary = "Import flows as a ZIP archive of yaml sources or a multi-objects YAML file.")
    @ApiResponse(responseCode = "204", description = "On success")
    public HttpResponse<Void> importFlows(
            @Parameter(description = "The file to import, can be a ZIP archive or a multi-objects YAML file") @Part CompletedFileUpload fileUpload)
            throws IOException {
        ...
        if (fileName.endsWith(".yaml") || fileName.endsWith(".yml")) {
            ...
            throw new IllegalArgumentException(
                    "Cannot import file of type " + fileName.substring(fileName.lastIndexOf('.')));
        }
    ...
    protected List<FlowWithSource> setFlowsDisableByQuery(String query, String namespace, List<String> labels,
            boolean disable) {
        return flowRepository
                .findWithSource(query, tenantService.resolveTenant(), namespace, RequestUtils.toMap(labels))
                .stream()
